		...
		if ( isset( $_GET['pay_for_order'] ) && isset( $_GET['key'] ) && isset( $wp->query_vars['order-pay'] ) ) {
			...
			if ( wcs_get_objects_property( $order, 'order_key' ) == $order_key && $order->has_status( array( 'pending', 'failed' ) ) && wcs_order_contains_renewal( $order ) ) {
				...
				foreach ( $subscriptions as $subscription ) {
					...
					if ( empty( $subscription ) || ! $subscription->has_status( array( 'on-hold', 'pending', 'late-payment', 'late-payment-30', 'late-payment-60', 'late-payment-90', 'late-payment-120', 'late-payment-150', 'late-payment-180', 'fraud', 'bad-payment', 'active'  ) ) ) {
